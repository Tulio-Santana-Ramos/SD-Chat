    ...
    while(!leave_flag){
        ...
        if(receive > 0){
            ...
            // Comando unmute
                ...
                receive = recv(cliente->fd_cliente, mensagem, LIMITE_MENSAGEM, 0);  // buffer "vazio" representando o fim de "/file <nome_arquivo>"
                ...
                while (true) {
                    ...
                    if (strlen(mensagem) == 0) break;
                    // fwrite(mensagem, 1, receive, arquivo);
                    fprintf(arquivo, "%s", mensagem);
                }
                fclose(arquivo);
                cout << cliente->nickname << " enviou um arquivo (verifique " << nomeArquivo << ")\n";
             }
